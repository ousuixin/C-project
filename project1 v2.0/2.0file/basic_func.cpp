    ...
    if (key == 'y') {
		...
		for (int i = 2; i >= 0; --i) {
            while (1) {
                ...
                ss >> s;
                if (ss >> s) {
                    cout << "~~ Your input is error, the Password should not have any ' '." << "\n"
                    	 << "~~ Please input Password agan without any ' '."
                         << endl;
                } else {
                    password = s;
                    break;
...
int choose_function() {
    ...
    while (1) {
        ...
        ss >> s;
        if (!(s[0] >= '0'&&s[0] <= '9'&&s.size()==1) || ss >> s) {
            cout << "~~ Your input is error, please only input a right number!"
                 << endl;
        } else {
            number = s[0] - '0';
            break;
	...
	if (number != 1 && number != 2 && number != 0 && !ret) {
		cout << "~~ Your input is error, please only input only one number between 0~2!"
		     << endl;
		choose_function();
		return 1;
...
//(2)
    	...
    	while (1) {
    	    ...
    	    if (ss >> num) {
	    	    if (ss >> s) {
    		        cout << "~~ Your input is error, please only input only one number every time!"
    		             << endl;
    	    	} else 
    	        	break;
    	    } else {
    	    	cout << "~~ Your input is error, please input only one number!" << endl;
    	    }
    	}
		if (num) {
		...
		for (int i = 0; i < num; i++) {
            while (1) {
                ...
                ss >> s;
                if (ss >> s) {
                    cout << "~~ Your input is error, Please input only one name every time.(there are no ' ' in a name)" 
                         << endl;
                } else if (s.size() > 12) {
                	cout << "~~ Your input is error, there are at most 12 letter in a name! please input the right one."
                         << endl;
                } else {
                    name = s;
                    break;
...
//(3)
		...
		for (int i = 0; i < num; ++i) {
			cout << "~~ Please input name numbers, prime_costs, and selling_price of the goods." << '\n'
				 << "~~ You should input only one set of data every time." << '\n'
				 << "~~ And the data should be list in one line, separated by space (' ')."
				 << endl;
    		while (1) {
    		    cout << "$$ ";
    		    stringstream ss;
    		    ...
    		    if (!(ss >> count) || !(ss >> prime_costs) || !(ss >> selling_price)) {
    		    	cout << "~~ Your input is error, please input the right one." << endl;
    		    	continue;
    		    }
    		    if (ss >> s) {
    		        cout << "~~ Your input is error, please only input only one set of data one time!"
    		             << endl;
    		    } else {
    		        break;
    		    }
...
//(7)
		...
		while (1) {
            ...
            ss >> s;
            if (ss >> s) {
                cout << "~~ Your input is error, the id should not have any ' '." << "\n"
                     << "~~ Please input id agan without any ' '."
                     << endl;
            } else {
                break;
            }
		...
		cout << "~~ Your input is error, please only input numbers between 0~7!"
		     << endl;
		choose_function();
	}
	return 1;
